...
export default function VoiceEnhancementFeature() {
  ...
  const features = config.mainNav
    ...
    .map(item => ({
      ...
      path: `/voice-enhancement${item.path.startsWith('/') ? item.path : `/${item.path}`}`,
      description: item.subTitle,
      icon: getFeatureIcon(item.path),
    }));

  function getFeatureIcon(path: string) {
    if (path.includes('podcast')) return 'headphones';
    if (path.includes('meeting')) return 'users';
    if (path.includes('voice-memo')) return 'mic';
    if (path.includes('interview')) return 'video';
    return 'headphones';
  }

  return (
    <section aria-labelledby="voice-enhancement-features-title" className="w-full py-16">
      {/* Section Header */}
      <div className="mb-16 text-center">
        <h2
          id="voice-enhancement-features-title"
        ...
        >
          {isZhHans ? '探索更多语音增强应用场景' : 'Discover More Voice Enhancement Use Cases'}
        </h2>
        <p className="mx-auto max-w-3xl text-xl text-gray-600 dark:text-gray-300">
          {isZhHans 
            ? '针对不同音频类型和使用场景，我们提供专业的语音增强解决方案，满足您的各种音频处理需求'
            : 'Specialized voice enhancement solutions for different audio types and use cases, meeting all your professional audio processing needs'
          }
        </p>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 sm:grid-cols-2 lg:grid-cols-4">
        {features.map((feature, idx) => {
          const Icon = Icons[feature.icon] || Icons.headphones;
          return (
            <Link
              key={idx}
              href={feature.path}
      ...
      <div className="mt-16">
        <div className="mx-auto max-w-4xl rounded-3xl border border-gray-200/50 bg-white/80 p-8 backdrop-blur-sm dark:border-gray-600/50 dark:bg-gray-800/80">
          <h3 className="mb-6 text-center text-2xl font-bold text-gray-900 dark:text-white">
            {isZhHans ? '为什么选择我们的语音增强工具？' : 'Why Choose Our Voice Enhancement Tool?'}
          </h3>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <div className="text-center">
              ...
              <h4 className="mb-2 font-semibold text-gray-900 dark:text-white">
                {isZhHans ? 'AI 智能处理' : 'AI Smart Processing'}
              </h4>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                {isZhHans ? '先进的 AI 算法自动优化音质' : 'Advanced AI algorithms automatically optimize audio quality'}
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <h4 className="mb-2 font-semibold text-gray-900 dark:text-white">
                {isZhHans ? '专业级质量' : 'Professional Quality'}
              </h4>
              <p className="text-sm text-gray-600 dark:text-gray-300">
                {isZhHans ? '达到广播级别的音频质量标准' : 'Achieves broadcast-grade audio quality standards'}
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">
                {isZhHans ? '分钟级处理，即时获得结果' : 'Minute-level processing, instant results'}
              </p>
            </div>
          </div>
        </div>
